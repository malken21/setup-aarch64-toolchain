...
> [!IMPORTANT]
> この Action は **AArch64 (ARM64) アーキテクチャの GitHub Actions ランナー**（例: `ubuntu-24.04-arm`）でのみ動作します。x86_64 ランナーでは動作しません。
...
AArch64 アーキテクチャのランナー（`ubuntu-24.04-arm` 等）を指定して実行してください。
